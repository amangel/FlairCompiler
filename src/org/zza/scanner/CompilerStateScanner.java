...
public class CompilerStateScanner {
    ...
    private CompilerToken makeToken(String token, String state)
        ...
        if (state.equalsIgnoreCase("identifier")) {
            ...
            return new CompilerToken(">=", token,
                    CompilerToken.GREATER_THAN_OR_EQ);
        ...
        try {
            if (!(toReturn.compareTo(max) == -1 && toReturn.compareTo(min) == 1)) {
                throw new LexicalException(
                        "LexicalException: Integers must be strictly less than "
            ...
            throw new LexicalException(
                    "LexicalException: Integers must be strictly less than "
                            + max + " and greater than " + min + ". Received "
                            + token);
        }
        return token;
    }
    ...
    private CompilerToken processIdentifier(String token)
        ...
        if (reservedStrings.contains(token)) {
            if (token.equalsIgnoreCase("program")) {
                ...
                return new CompilerToken("function", token,
                        CompilerToken.FUNCTION);
                
            } else if (token.equalsIgnoreCase("integer")) {
                return new CompilerToken("integer declaration", token,
                        CompilerToken.INTEGER_DECLARATION);
                
            } else if (token.equalsIgnoreCase("real")) {
                return new CompilerToken("real_declaration", token,
                        CompilerToken.REAL_DECLARATION);
        ...
        if (token.length() <= 256) {
            return new CompilerToken("identifier", token,
                    CompilerToken.IDENTIFIER);
        } else {
            throw new LexicalException(
                    "Identifier names must be under 256 characters long. "
                            + token.length() + " is too many.");
        }
    ...
    private void createScannerMaps() {
        ...
        for (char s : "abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ"
                .toCharArray()) {
        ...
        reservedStrings.addAll(Arrays.asList(new String[] { "program", "var",
                "return", "function", "integer", "real", "begin", "end", "if",
                "then", "else", "while", "do", "print" }));
